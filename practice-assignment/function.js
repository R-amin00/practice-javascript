...
const files = ['file.json', 'file.css', 'file.html', 'file.js', 'file.scss', 'file.xml'];
function isJavaScriptFile (files) {
    for (let i = 0; i < files.length; i++) {
        const file = files[i];
        let lastIndex = file.lastIndexOf(".");
        let extension = file.substring(lastIndex + 1);
        let javaScriptExtention = "js";
        // console.log(extension)
        if(extension === javaScriptExtention) {
            return true;
        }
    }
}
let file = isJavaScriptFile(files);
console.log(file)
